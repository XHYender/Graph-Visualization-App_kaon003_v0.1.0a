...
FILE=o.path.dirname(o.path.abspath(__file__)) #工作环境目录
...
class Limb:
    ...
    def __init__(self,S=None,N=2,fixed=True,joints_size_ratio=10,limbs_lenth_ratio=None,delta = 1,lenths:list=None,joints:list=None,iterate_num:int=5):
        ...
        if lenths is None and limbs_lenth_ratio is not None:
            self.lenths = [self.limbs_lenth_ratio*(1+2**(-1-i)) for i in range(N)][::-1]
        else:
            self.lenths=lenths
    ...
    def iterate(self,S:Vector,T:Vector):
        for _ in range(self.iterate_num):  #5次迭代上限防止找不到解决方案（目标点在可触范围之外导致的无限迭代）
            if T.distanceTo(self.joints[-1]) <= self.delta:break
            self.fabrik(S,T)
...
class Framework:
    ...
    """
    def __init__(self,title:str,width:int,height:int,target_fps=60):
        self.root=tk.Tk()
    ...
    def createWindow(self):
        ...
        #移除窗口边框
        self.root.iconbitmap(FILE+'/assets/enchantingtable0.ico')
        #窗口图标 (同目录.ico文件)
    ...
    def createSubTextWindow(self):
        ...
        self.sub_window_for_text.iconbitmap(FILE+'/assets/ebook_writable.ico')
...
class Aplication(Framework):
    ...
    def __init__(self,title:str='Graph',width:int =1080,height:int=720,save_id:int=0):
        ...
        self.test_limb = Limb(S=self.S,N=25,fixed=False,joints_size_ratio=20,limbs_lenth_ratio=20)
        #初始化test_limb
    ...
    def update(self):
        ...
        if self.scene_mode.mode==0:             #'for limbs'
            if self.test_limb_mode.mode == 1:
                self.makeTestLimbMoveTo(self.pointer_position)         
                
            elif self.test_limb_mode.mode == 2:
                self.makeTestLimbMoveInSOSTo(self.pointer_position)    
    ...
    if 1:   #{updatesFunctions}
        ...
        def makeNodeLightSwitch(self):
            if self.temp_timer_for_ignite is not None:  #长按点亮node部分
                if self.temp_timer_for_ignite+t.time() >=0.5:    #0.5s
        ...
        def detectPointerFocus(self):
            ...
            self.pointer_focus[1]=None             #每一帧检测前清空当前选择的对象
            if self.nodes:                              #节点部分
                for id,node in self.nodes.items():
                    if node.state==2:continue                   #跳过被选中的节点
                    node.low()                          #将节点的三态改为低姿态
                    dx=self.pointer_position.x-node.position.x
                    dy=self.pointer_position.y-node.position.y
                    r=1.5*node.r                        #敏感范围：1.5倍半径
                    if abs(dx+dy)<= r and abs(dx-dy)<= r:      #正方形包围框粗选
                ...
                if self.lines and self.pointer_selected[0]==0:            #线部分
                    p=self.pointer_position
                    for id,line in self.lines.items():
                        if line.state == 2:continue
                        line.low()
            ...
            if self.textblanks:                         #textblank
                for node_id,textblank in self.textblanks.items():
                    if textblank.state==2 or textblank.bbox is None:continue
                    textblank.low()
        ...
        def saveDatas(self,file_id:int=0):
            ...
            with open(f'{FILE}/saves/save{file_id}.json','w') as save_file:
                j.dump(save,save_file)
                print(f'save as {FILE}/saves/save{file_id}.json , succeeded!')

        def readDatas(self,file_id:int=0):
            """读取存档for nodes"""
            try:
                with open(f'{FILE}/saves/save{file_id}.json','r') as read_file:
                    ...
                    print(f'read at {FILE}/saves/save{file_id}.json , succeeded!')
...
def GetSaveID(file_name:str,els:int=0)->str:
    try:
        with open(file=file_name,mode='r') as txt:
            w=''.join([i for i in txt.read() if i in '1234567890'])
            if w:
                return int(w)
            else:
                with open(file=file_name,mode='w') as txt:
                    txt.write(str(els))
                return els
    except FileNotFoundError:
        print(f'no file named \"{file_name}\" for save id !')
        with open(file=file_name,mode='x') as txt:
            txt.write(str(els))
        return els

#main
if __name__ == "__main__":
    save=GetSaveID(FILE+'/save_number.txt')
    a=Aplication(save_id=save)
    a.run()