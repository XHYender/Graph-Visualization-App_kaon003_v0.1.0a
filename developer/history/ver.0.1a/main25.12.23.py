...
class Node(TriStates):
    def __init__(self,p:Vector=Vector(),text:str='None',r=15,id:int=0,light=None):
        super().__init__()
        self.position = p
        self.text=text
        self.r=r
...
class DisciplineAPP(Framework):
    ...
    def __init__(self,title:str='附魔台[Debug]',width:int =1080,height:int=720):
        ...
        self.temp_positions_for_drag={}     #用于中键拖拽画面
        self.temp_focus_target=Vector()
        #初始化临时量

        self.foucus_center=Vector()         #[bug]目前是在makeCameraFocusOn方法中提前实现。
        self.focus_mover=SOS(Vector(),1,0.9,1)
        #初始化focus动画
    ...
    def onResize(self,event):
        ...
        if event.widget == self.root:
            ...
            self.foucus_center=Vector(self.window_width//2,self.window_height//2)
            #获取当前画布大小
            self.render()
    ...
    def onMouseLClick(self,event):
        if self.foucus_player_mode == 1:    #聚焦过程中禁用鼠标行为
            return 0
    ...
    def onMouseLDoubleClick(self,event):
        if self.pointer_focus[0]==1:        #点亮Node
            self.nodes[self.pointer_focus[1]].light.switch()

        if self.pointer_focus[0]==2 and self.lines[self.pointer_focus[1]].heading:        #翻转Line
            sor_id=self.lines[self.pointer_focus[1]].source.id  #维护邻接表
            tar_id=self.lines[self.pointer_focus[1]].target.id
            self.adjoinGraphDel(sor_id,tar_id)
            self.adjoinGraphAdd(tar_id,sor_id,self.pointer_focus[1])

            self.lines[self.pointer_focus[1]].flip()
            self.lines[self.pointer_focus[1]].updateVector()
    ...
    def onMouseMClick(self,event):
        ...
        if self.pointer_focus[0]==3 and self.foucus_player_mode == 0:          #Canva
            ...
            if self.lines[self.pointer_focus[1]].heading:
                self.makeCameraFocusOn(self.lines[self.pointer_focus[1]].target.position)

        elif self.pointer_focus[0]==1:        #Node
            self.makeCameraFocusOn(self.nodes[self.pointer_focus[1]].position)

    def onMouseMDrag(self,event):
        self.onMouseMove(event)
        if self.pointer_selected[0]==3:   #Canva
            self.makeAllNodesMove(self.pointer_selected[1],self.pointer_position)
    ...
    def update(self):
        ...
        if self.scene_mode.mode==1:             #'for nodes
            ...
            if self.foucus_player_mode == 1:    #聚焦动画
                self.focus_mover.iterate(self.foucus_center,self.delta_time)
                self.makeAllNodesMove(self.temp_focus_target,self.focus_mover.prp)
                if Vector(-0.5,-0.5)<=self.focus_mover.prp-self.foucus_center<=Vector(0.5,0.5):     #误差0.5
                    self.foucus_player_mode.mode =0
    ...
    def makeAllNodesMove(self,source:Vector,target:Vector):
        shift=target-source
        for id in self.nodes.keys():
            self.nodes[id].position = self.temp_positions_for_drag[id]+shift

    def makeCameraFocusOn(self,target:Vector):
        self.setFocusCenter()
        self.temp_focus_target=target
        for id,node in self.nodes.items():
            self.temp_positions_for_drag[id]=node.position

        self.focus_mover.prp =self.focus_mover.p=self.focus_mover.pp= target
        self.focus_mover.prs = Vector()
        #初始化focus mover sos的开始位置

        self.foucus_player_mode.switch()
            ...
            if self.lines:            #line
                for id,line in self.lines.items():
                    if line.state == 2:
                        continue
                    line.low()
                    if line.source.position<self.pointer_position<line.target.position or line.source.position>self.pointer_position>line.target.position:  #边界框-粗选
                        sp=line.source.position-self.pointer_position
                        lst=abs(line.vector)
                        if abs((sp)*line.vector.verticle())<=lst*line.width*2:  #敏感范围：2倍线宽
    ...
    def setFocusCenter(self):
        self.foucus_center=Vector(self.canva.winfo_width()//2,self.canva.winfo_height()//2)

    def saveDatas(self,file_id:int=0):
        """保存存档for nodes"""
        if self.scene_mode != 1 or self.foucus_player_mode !=0: #只能在安全状态下保存读取
            print('save failed!')
            return 0
        save={
              ...
              'nodes':[[node.position.x,node.position.y,node.text,node.r,node.light.mode,id] for id,node in self.nodes.items()],
    ...
    def readDatas(self,file_id:int=0):
        ...
        try:
            with open(f'saves/save{file_id}.json','r') as read_file:
                if self.scene_mode != 1 or self.foucus_player_mode !=0:
                    print('read failed!')
                    return 0
                ...
                self.nodes = {n[5]:Node(p=Vector(n[0],n[1]),text=n[2],r=n[3],id=n[5],light=n[4]) for n in reads['nodes']}
                self.lines = {l[5]:Line(self.nodes[l[0]],self.nodes[l[1]],heading=l[2],weight=l[3],width=l[4],id=l[5]) for l in reads['lines']}
