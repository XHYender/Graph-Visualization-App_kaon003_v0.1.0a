...
class Node(TriStates):
    def __init__(self,p:Vector=Vector(),text:str='Text',r=15,id:int=0,light=None,title:str='Node:'):
        super().__init__()
        self.position = p
        self.r=r
        self.id=id
        self.text=text
        self.title=title+str(id)
        self.light=Mode()
...
class Textblank(TriStates):
    def __init__(self,text:str='Default Text',anchor:Vector=Vector(),hide=False,node_id:int=-1,rod=0):
        super().__init__()
        self.text=text
        self.anchor=anchor
        self.font=('黑体',16)
        ...
        self.rod=rod            #杆长
        self.bbox=None     #包围盒坐标：左上，右下

class Limb:
    """Forward And Back Reaching Inverse Kinematics"""
    def __init__(self,S=None,N=2,fixed=True,joints_size_ratio=10,limbs_lenth_ratio=None,delta = 1,lenths:list=None,joints:list=None):
...
class Framework:
    ...
    def createSubTextWindow(self):
        ...
        self.sub_window_text_blank=tk.Text(self.sub_window_for_text, wrap=tk.WORD, 
                                  bg="#272728",fg="lightblue",        #5EC8E3
                                  insertbackground="#FFFFFF",
                                  selectbackground="#93B9CA",
                                  selectforeground="#2E3030",
                                  font=('黑体',14),undo=True)
...
class Aplication(Framework):
    ...
    def __init__(self,title:str='附魔台[Debug]',width:int =1080,height:int=720):
        ...
        self.test_limb = Limb(S=self.S,N=1,fixed=True,joints_size_ratio=8,limbs_lenth_ratio=20)
        #初始化test_limb
        ...
        def onMouseLDrag(self,event):
            ...
            if self.pointer_selected[0]!=0:
                if self.pointer_selected[0]==1:     #Node
                    ...
                    if self.nodes_mode==0 or self.textblanks[self.pointer_selected[1]].hide:
        ...
        def onMouseLRelease(self,event):
            if self.pointer_selected[0]==1:         #Node
                ...
                if self.temp_timer_for_ignite is not None:  #长按点亮node部分
                    ...
                    if self.temp_timer_for_ignite >=0.8:    #0.8s
        ...
        def onMouseLDoubleClick(self,event):
            ...
            if self.pointer_focus[0]==1:        #node:打开/关闭文本框(only for title)
                if self.textblanks.get(id,None) is None:    #初始化
                    ...
                    self.setTextblanks(id)
                    self.textblank_rods[id]=Limb(joints=[self.textblanks[id].anchor,self.nodes[id].position],N=1,fixed=False,lenths=[self.textblanks[id].rod+self.nodes[id].r])
    ...
    if 1:
        ...
        def swicthTextWindow(self,node_id:int=None):
            if self.text_mode==0 and node_id is not None:   #打开子窗口
                ...
                try:
                    ...
                    self.setTextblanks(self.temp_node_id_for_text_or_focus)
        ...
        def setTextblanks(self,node_id:int=-1):
            self.textblanks[node_id].text=self.nodes[node_id].title
            self.textblanks[node_id].anchor=Vector(self.textblanks[node_id].rod+self.nodes[node_id].r,0)+self.nodes[node_id].position            #初始化锚杆
        ...
        def showNodes(self):
            if self.nodes:
                ...
                if self.temp_node_id_for_text_or_focus is not None:     #聚焦/编辑
                    ...
                    r=self.nodes[self.temp_node_id_for_text_or_focus].r
