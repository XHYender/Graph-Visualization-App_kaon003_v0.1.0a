...
class Text(TriStates):
    def __init__(self,text:str='Default Text',anchor:Vector=Vector(),size:Vector=Vector()):
        super().__init__()
        self.text=text
        self.anchor=anchor
        self.size=size
...
class Aplication(Framework):
    ...
    if 1:
        ...
        def onKeyPressD(self,event):
            if self.pointer_focus[0] !=0 and self.pointer_focus[0] !=3:
                print('a')
        ...
        def onMouseLDoubleClick(self,event):
            ...
            if self.pointer_focus[0]==2 and self.lines[self.pointer_focus[1]].heading:        #翻转Line
                sor_id=self.lines[self.pointer_focus[1]].source.id  
                tar_id=self.lines[self.pointer_focus[1]].target.id
                self.adjoinGraphDel(sor_id,tar_id)
                self.adjoinGraphAdd(tar_id,sor_id,self.pointer_focus[1])#维护邻接表
        ...
        if self.scene_mode.mode==1:             #'for nodes
            ...
            if self.focus_player_mode == 1:    #聚焦动画
                ...
                if not self.focus_mover.end:
                    self.makeAllNodesMove(self.temp_focus_target,self.focus_mover.iterCubic(self.delta_time))
                else:
    ...
    if 1:
        ...
        def setAdjoinGraphWithWeight(self):
            self.adjoin_graph_with_weights={ sor:{tar:self.lines[id].weight for tar,id in tars.items()} for sor,tars in self.adjoin_graph.items() }
            print(self.adjoin_graph_with_weights)
