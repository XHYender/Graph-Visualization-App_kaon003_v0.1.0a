...
class Node(TriStates):
    def __init__(self,p:Vector=Vector(),text:str='None',r=15,id:int=0):
        super().__init__()
        self.position = p
        self.text=text
        self.r=r
...
class DisciplineAPP(Framework):
    ...
    def onMouseLDoubleClick(self,event):
        ...
        if self.pointer_focus[0]==2:        #翻转Line
    ...
    def adjoinGraphAdd(self,sor_id:int,tar_id:int,line_id:int):
        if self.adjoin_graph.get(sor_id,True):
            self.adjoin_graph[sor_id]={}
        self.adjoin_graph[sor_id][tar_id]=line_id

    def adjoinGraphDel(self,sor_id:int,tar_id:int)->int:
